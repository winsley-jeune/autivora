...
- Anthropic calls are cost-routed: Sonnet handles routine generation and market synthesis over compact DataForSEO evidence; Opus is reserved for independent high-risk catalog verification. Catalog audits do not invoke Anthropic web search (which duplicated DataForSEO and caused large hidden cache/input usage), audit output is capped at 8k tokens, verification at 4k, actual usage is recorded, and worst-case preflight reservations enforce default $2 daily and $15 monthly AI limits before a request is sent.
