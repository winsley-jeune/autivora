...
- Local inference defaults to Ollama + Qwen 3.5 9B with schema-constrained output, deterministic validation, a measured 48K context, and one model job at a time. DataForSEO supplies live SERP/Shopping evidence to research tasks. Schema-valid examples are retained in SQLite for later LoRA curation. Anthropic remains an optional configuration fallback, not a runtime requirement.
