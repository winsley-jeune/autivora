...
async function main() {
  ...
  const { orders } = await shopifyApi("GET", `orders.json?status=any&financial_status=paid&created_at_min=${encodeURIComponent(isoDaysAgo(7))}&limit=250&fields=id,created_at,total_price,line_items`);
  ...
  const headline = todays.length
    ...
    : `0 sales today`;
  const line = `[scoreboard ${today()}] ${headline} | 7d: ${orders.length} orders / $${weekRevenue.toFixed(2)} | North Star ${todays.length}/${northStar}`;
