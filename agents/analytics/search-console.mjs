...
async function fetchSearchAnalytics(siteUrl, token, dimensions, rowLimit = 1000) {
  ...
  const res = await fetch(endpoint, {
    ...
    body: JSON.stringify({ startDate: dateNDaysAgo(DAYS), endDate: dateNDaysAgo(1), dimensions, rowLimit }),
  });
...
export async function pullSearchConsole() {
  ...
  return { siteUrl: GSC_SITE_URL, windowDays: DAYS, queries, pages, pageQueries };
}
